...
import { ConnectionDetails } from '@/app/api/voice-connection/route';
import { AppConfig } from '@/lib/voice/types';
...
export default function useConnectionDetails(appConfig: AppConfig) {
  ...
  const fetchConnectionDetails = useCallback(async () => {
    ...
    const url = new URL(
      process.env.NEXT_PUBLIC_CONN_DETAILS_ENDPOINT ?? '/api/voice-connection',
    ...
    try {
      const res = await fetch(url.toString(), {
        ...
        body: JSON.stringify({
          index_name: appConfig.sandboxId || 'pdf-index', // Use sandboxId as index name
          agent_name: appConfig.agentName || 'rag-mentor',
        }),
      });
      data = await res.json();
    } catch (error) {
