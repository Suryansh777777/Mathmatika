      ...
      <div className="self-stretch h-auto flex flex-col md:flex-row justify-between items-stretch pr-0 pb-8 pt-0">
        <div className="h-auto p-4 md:p-8 flex flex-col justify-start items-start gap-8">
          <div className="self-stretch flex justify-start items-center gap-3">
            <div className="text-center text-[#49423D] text-xl font-semibold leading-4 font-sans">{name}</div>
          </div>
          ...
          <div className="flex justify-start items-start gap-4">
            {footer.social.map((social) => (
              <div key={social.name} className="w-6 h-6 relative overflow-hidden">
        ...
        <div className="self-stretch p-4 md:p-8 flex flex-col sm:flex-row flex-wrap justify-start sm:justify-between items-start gap-6 md:gap-8">
          {Object.entries(footer.links).map(([category, links]) => (
            <div key={category} className="flex flex-col justify-start items-start gap-3 flex-1 min-w-[120px]">
              <div className="text-[rgba(73,66,61,0.50)] text-sm font-medium leading-5 font-sans capitalize">
                {category}
              </div>
